...
# ---- COOKING/MEAL 슬롯에 자동으로 메뉴 후보 붙이기 ----
...
def coordinator_page():
    ...
    example_text = (
        "08:00 · [MORNING_BRIEFING] 오늘 날씨와 일정 간단 안내\n"
        "10:00 · [GENERAL] 옷 입기 연습하기\n"
        "12:00 · [MEAL] 라면 또는 카레 중 하나 먹기\n"
        "15:00 · [LEISURE] 쉬는 시간 갖기\n"
        "18:00 · [HEALTH] 앉아서 하는 운동하기\n"
        "22:00 · [NIGHT_WRAPUP] 하루 마무리 인사하기\n"
    )
    raw = st.text_area(
        ...
        with st.expander(
                ...
                if menus:
                    ...
                    for m_idx, menu in enumerate(menus):
                        ...
                        with cols[1]:
                            ...
                            default_img_query = menu.get("img_query") or menu_name
                            ...
                            if st.button("사진 추천 받기", key=f"search_img_{idx}_{m_idx}"):
                                with st.spinner("사진을 찾는 중입니다."):
                                    try:
                                        img_results = search_and_filter_food_images(img_query, max_images=8)
                                    except Exception:
                                        img_results = []
                            ...
                            if img_results:
                                if clickable_images is None:
                                    ...
                                    chosen = st.radio(
                                        "사진 선택",
                                        options=list(range(len(img_results))),
                                        format_func=lambda i: img_results[i].get("title", f"사진 {i+1}"),
                                        key=f"img_radio_{idx}_{m_idx}",
                                    )
                                    chosen_url = img_results[chosen]["url"]
                                    if st.button("선택한 사진 적용", key=f"apply_img_{idx}_{m_idx}"):
                                        try:
                                            local_path = download_image_to_assets(chosen_url, prefix="food")
                                            st.session_state[SCHEDULE_STATE_KEY][idx]["menus"][m_idx]["image"] = local_path
                                            changed = True
                                        except Exception:
                                            pass
                                else:
                                    thumbs = [x.get("thumbnail") or x.get("url") for x in img_results]
                                    chosen_idx = clickable_images(
                                        thumbs,
                                        titles=[x.get("title", "") for x in img_results],
                                        div_style={"display": "flex", "flex-wrap": "wrap", "gap": "6px"},
                                        img_style={"height": "80px"},
                                        key=f"click_imgs_{idx}_{m_idx}",
                                    )
                                    if chosen_idx is not None and 0 <= chosen_idx < len(img_results):
                                        chosen_url = img_results[chosen_idx]["url"]
                                        try:
                                            local_path = download_image_to_assets(chosen_url, prefix="food")
                                            st.session_state[SCHEDULE_STATE_KEY][idx]["menus"][m_idx]["image"] = local_path
                                            changed = True
                            ...
                            if st.button("요리/식사 영상 추천 받기", key=f"search_yt_food_{idx}_{m_idx}"):
                                with st.spinner("관련 영상을 찾는 중입니다."):
                                    try:
                                        yt_results = search_cooking_videos_for_dd_raw(yt_query, max_results=6)
                                    except Exception:
                                        yt_results = []
            ...
            # ---------------- HEALTH ----------------
                ...
                if st.button("운동 영상 추천 받기", key=f"search_yt_health_{idx}"):
                    with st.spinner("운동 영상을 찾는 중입니다."):
                        try:
                            yt_results = search_exercise_videos_for_dd_raw(health_yt_query, max_results=6)
                        except Exception:
                            yt_results = []
                        st.session_state[yt_key] = yt_results
            ...
            # ---------------- CLOTHING ----------------
                ...
                if st.button("옷 입기 영상 추천 받기", key=f"search_yt_clothing_{idx}"):
                    with st.spinner("관련 영상을 찾는 중입니다."):
                        try:
                            yt_results = search_clothing_videos_for_dd_raw(clothing_yt_query, max_results=6)
                        except Exception:
                            yt_results = []
                        st.session_state[yt_key] = yt_results
